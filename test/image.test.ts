...
describe('binarize', () => {
  it('thresholds bright pixels to white and dark pixels to black', () => {
    const out = binarize(buf([[255, 255, 255], [0, 0, 0], [200, 200, 200], [50, 50, 50]]), {
      threshold: 128,
      ...LUMA,
      invert: false,
    });
  ...
  it('inverts when requested (white HUD text → black glyphs for OCR)', () => {
    const normal = binarize(buf([[255, 255, 255], [0, 0, 0]]), { threshold: 128, ...LUMA, invert: false });
    const inverted = binarize(buf([[255, 255, 255], [0, 0, 0]]), { threshold: 128, ...LUMA, invert: true });
  ...
  it('rejects colored pixels even when bright (white-text keying)', () => {
    ...
    const out = binarize(buf([[255, 255, 255], [255, 140, 40], [180, 180, 180]]), {
      threshold: 120,
      chromaTol: 70,
      invert: false,
    });
